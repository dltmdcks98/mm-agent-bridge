...
- 실행기 어댑터: `mock`, `codex_cli`, `claude_cli` 선택 가능
...
- 실제 실행을 원하면 `.env`에서 `MM_BRIDGE_EXECUTOR_BACKEND=codex_cli` 또는 `claude_cli`로 설정하세요.
...
- `MM_BRIDGE_EXECUTOR_BACKEND`: `mock`(기본), `codex_cli`, `claude_cli`
- `MM_BRIDGE_CODEX_CLI_COMMAND`: Codex CLI 실행 파일명 (기본 `codex`)
- `MM_BRIDGE_CODEX_CLI_ARGS`: Codex CLI 추가 인자(공백 구분)
- `MM_BRIDGE_CLAUDE_CLI_COMMAND`: Claude CLI 실행 파일명 (기본 `claude`)
- `MM_BRIDGE_CLAUDE_CLI_ARGS`: Claude CLI 추가 인자(공백 구분)
